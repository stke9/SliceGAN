...
def CBatch(imgs,lbls, typ, l, sf, TI):
    if typ == 'self':
        ...
        data = np.empty([32 * 900, 3, l, l])
        labelset = np.empty([32 * 900, 3, 1, 1])
        p = 0
        nimgs = len(imgs)
        print('number of training imgs: ', nimgs, ' number of labels: ', 3)
        for img in imgs:
            img = np.load(img)
            print(img.shape)
            if len(img.shape) > 3:
            ...
            for i in range((32 // nimgs) * 900):
                ...
                for cnt, phs in enumerate(phases):
                     ...
                     img1[img[x:x + l, y:y + l, z] == phs] = 1
                     data[p, cnt, :, :] = img1
                     labelset[p, cnt] = np.mean(img1)
                p += 1
                if i % 3600 == 0:
                    plt.imshow(data[p - 1, 0] + 2 * data[p - 1, 1])
                    plt.title(str(np.round(labelset[p-1,0],2))+' ' + str(np.round(labelset[p-1,1],2)))
